...
import {getActionsHistory, getConnectionHistory, getFiltredUsers, getStoresForAdmin} from "@/app/api";
import LogoutService from "@/app/service/LogoutService";
import locale from "antd/locale/fr_FR";
import {DataType} from "csstype";
import HistoryTable from "@/app/components/dashboardComponents/ConnectionHistory/components/HistoryTable";
...
function ConnectionHistory() {
    ...
    const [connectionHistoryList, setActionsHistoryList] = useState<DataType[]>([]);
    const [connectionHistoryListCount , setActionsHistoryListCount] = useState<number>(0);
    ...
    useEffect(() => {
        getConnectionsHistories();
    }, [searchParam]);

    function getConnectionsHistories() {
        setLoading(true);
        getConnectionHistory(searchParam).then((response) => {
            console.log('response : ', response);
            setActionsHistoryList(response.connectionsHistory);
            setActionsHistoryListCount(response.connectionsHistoryCount);
            setLoading(false);
        }).catch((err) => {
            setLoading(false);
            if (err.response) {
            ...
            <div className={`${styles.homePageContentTopHeader}`}>
                <h1 className={`mx-3`}>
                    ...
                    {userRole === 'ROLE_STOREMANAGER' && (
                        <>
                            Historique des connexions associés au magasin
                        </>
                    )}

                    {userRole === 'ROLE_CLIENT' && (
                        <>
                            Historique des connexions associés au client
                        </>
                    )}
                    {userRole === 'ROLE_EMPLOYEE' && (
                        <>
                            Historique des connexions associés au caissier
                        </>
                    )}
                ...
                <div className={`${styles.ticketsCardsMain}`}>
                    <div className={`${styles.ticketsCardsDiv} mb-5 px-4`}>
                                ...
                                <>
                                    <Col key={"resultTikcets"} className={`w-100 d-flex justify-content-between mt-3 px-4 ${styles.clientsTableColDashboard}`} xs={24} sm={24} md={24} lg={24} span={6}>
                                        <div className={`w-100 d-flex justify-content-between mt-3 px-4`}>
                                            ...
                                            <h6>
                                                {connectionHistoryList?.length}
                                                 <span className={`mx-2`}>
                                                     sur
                                                 </span>
                                                {connectionHistoryListCount}
                                        ...
                                        <div className={`${styles.clientsTableDashboard}`}>
                                            <HistoryTable key={connectionHistoryListCount} selectedStoreId={null} data={connectionHistoryList as any} ></HistoryTable>
                                        </div>
                        ...
                        {!loading && connectionHistoryListCount>0 &&
                            <Row className={`${styles.fullWidthElement} w-100 mt-5 justify-content-center`}
                                ...
                                <Pagination
                                    ...
                                    defaultCurrent={parseInt(searchParam.page)} total={connectionHistoryListCount}/>
                            </Row>
                        }
                    </div>
